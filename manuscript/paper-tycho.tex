...
models.  We are unable to distinguish between scenarios, as models for rim
widths incorporating electron cut-offs both reproduce the observed rim
drop-offs.  The best loss-limited fits require amplified magnetic fields of
$\abt200$--$1000 \muG$ and particle diffusion coefficients $\abt
1$--$100\times$ Bohm values, consistent with prior work on Tycho's SNR, whereas
magnetic damping fits permit magnetic fields of $\abt 10$--$300 \muG$ and
comparatively smaller diffusion coefficients (typically $\abt 1$--$10$).
Non-negligible diffusion coefficients, magnetic field strengths, and magnetic
damping lengthscales are somewhat degenerate in setting observed rim widths and
drop-offs.  A different approach will be needed to determine the presence of
magnetic damping downstream of supernova remnant shocks.
\end{abstract}
...
``knee'' \citep{vink2012}.  \note[Aaron]{the discussion of hadronic
acceleration isn't really relevant, just background information}
...
We first introduce our transport models for electron advection and diffusion
with continuous synchrotron losses.  We selected and measured rim widths at
several locations around Tycho's forward shock, verifying that selected rims
are free of thermal line emission.  Using our models for particle transport
(with DSA included, to first order, in a simple exponentially cut-off injected
electron spectrum), we predict width-energy dependence in the loss-limited and
magnetic damping scenarios to obtain estimates on magnetic field strength and
diffusion coefficient magnitude.  We discuss degeneracy in our model fits and
the implications for future studies of young supernova remnant shocks.
...
\citet{parizot2006} employed this model to estimate magnetic fields, diffusion
coefficients, and electron spectrum cut-offs for several historical SNRs.
Conveniently, equation~\eqref{eq:simp-mod} has analytic solution $f \propto
\exp(-x/a)$ where $a$ is a characteristic scale length.  The thin rim FWHM
predicted by equation~\eqref{eq:simp-mod} is:
...
The cut-off energy is determined by the diffusion coefficient $D$,
...
particle energy $E_h$.  Note that $\eta_h$ and $\eta$ are related as
...
\begin{align} \label{eq:ecut}
    \Ecut =
        ...
        \eta^{-1 / (1+\mu)} .
\end{align}
\citet{parizot2006} derive equation~\eqref{eq:ecut} in the case $\mu=1$
assuming a strong shock with compression ratio 4 and isotropic magnetic
turbulence on both sides of the shock.
...
values of $\mu$ (but recall that the specific electron energies associated with
$\eta_2$ will depend on $B$).
...
full model solutions to equation~\eqref{eq:full-mod}.
...
\begin{figure*}
    ...
    \caption{Spectra and fits from Regions 1 (top) and 14 (bottom) show varying
...
\begin{figure*}%[ht]
    ...
    \caption{Best fit profiles with measured FWHMs demarcated for each energy
        band in Region 1 (top) and Region 14 (bottom).  Energy bands increase
...
\begin{table}[ht]
    \scriptsize
    \centering
    \caption{Best loss-limited fits for Regions 1, 16
    \label{tab:fits-loss-ex}}
    \input{tab-fits-loss-ex.tex}
\end{table}
...
\begin{table}[ht]
    \scriptsize
    \centering
    \caption{Best magnetic damping fits for Regions 1, 16
    \label{tab:fits-damp-ex}}
    \input{tab-fits-damp-ex.tex}
\end{table}

% NOTE: USE CHISQR HERE, NOT REDUCED CHI SQUARED.
% Justification: comparing fits with parameters free/frozen
...
\begin{table}[ht]
    \tiny
    \centering
    \caption{Best model fits with magnetic damping, $\mu = 1$
    \label{tab:fits-damp}}
    \input{tab-fits-all-summary.tex}
\end{table}

% TODO LIST DOFs in FINAL VERSION

% As above but with eta2=1 fixed!
\begin{table}[ht]
    \scriptsize
    \centering
    \caption{Best model fits with magnetic damping, $\mu = \eta_2 = 1$
    \label{tab:fits-damp}}
    \input{tab-fits-all-summary-eta2one.tex}
\end{table}
...
\begin{figure*}[ht]
    ...
    \caption{Full model rim profile width predictions for Regions 1
        (left) and 14 (right) with measured data.  Note that ordinate FWHM axes
        do not match and do not originate at zero, to better show model
...
\begin{figure*}[ht]
    ...
    \plottwo{figures/full-man_err-energywidth-01.pdf}{figures/full-man_err-energywidth-16.pdf} \\
    ...
    model fit with moderate damping ($a_b < 0.01$).
    \label{fig:fits}}
\end{figure*}
...
% Radio rims very much TBD!

%\begin{figure*}[ht]
%    \centering
%    \plotone{figures/full-man_err-prfs-radio_01.pdf}
%    \plotone{figures/full-man_err-prfs-radio_14.pdf}
%    \caption{Full model rim profile width predictions for Regions 1
%    (top) and 14 (bottom) with measured data.
%    \label{fig:rims-radio}}
%\end{figure*}
