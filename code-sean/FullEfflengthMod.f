c Sean Ressler's program for computing radial profiles
c of synchrotron thin rims, given parameters B0, eta2, mu
c B0 = magnetic field, eta2 = scaled diffusion coefficient at 2 keV
c mu = spectral index of diffusion coefficient power law dependence

c Reference: Ressler et al., ApJ, in press

c UPDATED by Aaron Tran for application to Tycho's SNR
c Summer 2014

c Major changes (so far, if documented):
c Retab, changes to some spacing/commenting conventions/whatever
c The tabs seem to be driving vim crazy
