...
def test_get_FWHM_all():
    """Check that error calculation is at least self-consistent"""
    x = np.linspace(-2, 4, 100)
    y = gaussian(x, 1., 1.)
    y[np.argmax(y)] = 0.9 * np.amax(y)  # To give an imperfect fit
    eps = np.sqrt(y)  # Poisson like error

    popt, pcov = sp.optimize.curve_fit(gaussian, x, y, p0=[1,1], sigma=eps,
                                       absolute_sigma=True)
    chi2_best = crvfit.chi2(x, y, eps, gaussian, popt)
    xi_neg, xi_pos = ffwhm.get_stretch_limits(x, y, eps, gaussian, popt, 1.,
                                              xconst=10)

    f_neg = ffwhm.stretch_func(gaussian, xi_neg, 1., xconst=10)
    f_pos = ffwhm.stretch_func(gaussian, xi_pos, 1., xconst=10)

    assert_almost_equal(chi2_best + 2.7, crvfit.chi2(x, y, eps, f_neg, popt))
    assert_almost_equal(chi2_best + 2.7, crvfit.chi2(x, y, eps, f_pos, popt))
