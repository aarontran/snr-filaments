...
def ind_first_min(y, **kwargs):
    ...
    **kwargs (window_len, window) passed to std_smooth
    ...
    inds_relmin = sp.signal.argrelmin(y_smooth)[0]
    if len(inds_relmin) is 0:
        return 0

    try:
        ind_max = get_ind_relmax(y, **kwargs)
    except ValueError:
        return 0

    # Find the largest value in inds_relmin, that's smaller than ind_max
    idx = np.searchsorted(np.sort(inds_relmin), ind_max)
    if idx == 0:  # All relmin indices are farther right than ind_max?
        return 0
    return inds_relmin[idx-1]

def get_ind_relmax(y, **kwargs):
    """Get index of largest relative maximum in smoothed data
    **kwargs (window_len, window) passed to std_smooth

    If no relative maximum found, throws ValueError
    """
    ysmooth = std_smooth(y, **kwargs)
    inds_relmax = sp.signal.argrelmax(ysmooth)[0]
