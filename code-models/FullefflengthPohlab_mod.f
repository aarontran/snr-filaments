! Sean's model code, with magnetic damping
! Modified by Aaron, no substantial changes.  Only editing whitespace
! for readability, for now
! I may introduce spaces in place of tabs, somewhat inconsistently...
! but I'll try to avoid it.  Revert to original version as needed.
...
!Physics Parameters
       ...
       ab =.05d0
       Bmin =5d-6
       rs = 2.96e19    !Modified from nufit code....distance is now 2.2 kpc, so rs is found from tan(943.7'')*2.2 kpc 
       alpha = .6d0
       s = 2d0*alpha+1d0
...
!Start tabulation of the electron distribution in a grid of r and the appropriate Es 
!for each r (E = sqrt(nu/c1/B(r)/x)
       ...
       call emisx(nu, B0, emistab, radtab,
     c  disttab, ab, Bmin, iradmax)
...
190	continue
	...
	if (i.eq.100.and.inu.eq.1) write (1002, *) rho, emis1
...
!	rmin = 1d0-(1d0-rmin)*((2d0**.25)**(-.05d0/4))
	...
	write(225,*) nugraph(n)/2.417989d17, fluxpeak(n), fluxplat(n), 
