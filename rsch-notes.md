...
Continued in a new file (as of Aug. 25, `rsch-notes-fall.md`)
...
### Interpolate B0-chisqr values from grid

Spline fitting in B0-chisqr space looks okay.  About half the time, it
doesn't get a better chisqr value.  Polynomial fit looks about the same.

Also, interpolation isn't too useful because we can't use our interpolated chisqr
values (especially when interp. chisqr shoots negative!).
What if we try a strictly interpolating spline?  Still risks overshooting...
just another source of noise.  If we want to nail down chisqr space we'd have
to perform the actual computation.
If we try to fit, we won't have the same rminarc values, which adds
fluctuation too...

_Verdict: forget it._
Only worry about fitting at best (grid) fit and error bounds.

Let a more careful fit from best grid values help figure out best parameters,
and remember that errors are very large anyways.
We're not doing a lot better than manual fitting here.
